package cn.yunluosoft.tonglou.utils.album;
...
import cn.yunluosoft.tonglou.model.ImageBucket;
import cn.yunluosoft.tonglou.model.ImageComparator;
...
public class AlbumHelper {
	...
	void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			...
			int mis = cur.getColumnIndexOrThrow(Media.DATE_ADDED);
			// Media.
			// 获取图片总数
...
//				cur.getLong(mis);
	...
	public List<ImageBucket> getImagesBucketList(boolean refresh) {
		...
		for (int i = 0; i < tmpList.size(); i++) {
			if (tmpList.get(i).imageList != null
					&& !"所有图片".equals(tmpList.get(i).bucketName)) {
				for (int j = 0; j < tmpList.get(i).imageList.size(); j++) {
					tmpList.get(i).imageList.get(j).isSelected = false;
					imageItems.add(tmpList.get(i).imageList.get(j));
				}
			}
		}
		Collections.sort(imageItems, new ImageComparator());
//		CoimageItems.
		imageBucket.bucketName = "所有图片";
		imageBucket.count = imageItems.size();
